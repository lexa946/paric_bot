...
from app.bot.keyboards import admin_keyboard, main_keyboard
from app.config import settings

router = Router()


@router.message(F.text == '🔑 Админ панель', F.from_user.id.in_([settings.ADMIN_ID]))
async def admin_panel(message: Message):
    await message.answer(
        ...
        reply_markup=admin_keyboard(user_id=message.from_user.id, first_name=message.from_user.first_name)
    )


@router.callback_query(F.data == 'back_home')
async def cmd_back_home(callback: CallbackQuery):
